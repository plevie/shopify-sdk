...
import com.shopify.model.ShopifyDraftOrdersRoot;
...
public class ShopifySdk {
    ...
    public ShopifyPage<ShopifyProduct> getCollectionProducts(final String collectionId, final String pageInfo,
            final int pageSize) {
    ...
    public ShopifyProducts getCollectionProducts(final String collectionId) {
        ...
        shopifyProducts.addAll(shopifyProductsPage);
        while (shopifyProductsPage.getNextPageInfo() != null) {
            shopifyProductsPage = getCollectionProducts(collectionId, shopifyProductsPage.getNextPageInfo(),
                    DEFAULT_REQUEST_LIMIT);
            LOGGER.info("Retrieved {} products for collection {} from page {}", shopifyProductsPage.size(), collectionId,
    ...
    public ShopifyCustomCollection getCustomCollection(final String collectionId) {
        ...
        final ShopifyCustomCollectionRoot shopifyCsutomCollectionRootResponse = response
    ...
    public ShopifySmartCollection getSmartCollection(final String collectionId) {
        ...
        final ShopifySmartCollectionRoot shopifySmartCollectionRootResponse = response
                .readEntity(ShopifySmartCollectionRoot.class);
    ...
    public ShopifyPage<ShopifySmartCollection> getSmartCollections(final int pageSize) {
        final Response response = get(getWebTarget().path(SMART_COLLECTIONS).queryParam(LIMIT_QUERY_PARAMETER, pageSize));
        final ShopifySmartCollectionsRoot shopifySmartCollectionsRoot = response.readEntity(ShopifySmartCollectionsRoot.class);
    ...
    public List<ShopifySmartCollection> getSmartCollections() {
        ...
        while (smartCollectionsPage.getNextPageInfo() != null) {
            smartCollectionsPage = getSmartCollections(smartCollectionsPage.getNextPageInfo(), DEFAULT_REQUEST_LIMIT);
    ...
    public ShopifyDraftOrder getDraftOrder(final String draftOrderId) {
    ...
    public ShopifyPage<ShopifyOrder> getOrders(final DateTime mininumCreationDate, final int pageSize) {
        final Response response = get(
                buildOrdersEndpoint().queryParam(STATUS_QUERY_PARAMETER, ANY_STATUSES).queryParam(LIMIT_QUERY_PARAMETER, pageSize)
    ...
    public ShopifyPage<ShopifyDraftOrder> getDraftOrders() {
        return getDraftOrders(DEFAULT_REQUEST_LIMIT);
    }

    public ShopifyPage<ShopifyDraftOrder> getDraftOrders(final int pageSize) {
        final Response response = get(buildDraftOrdersEndpoint().queryParam(STATUS_QUERY_PARAMETER, ANY_STATUSES)
                .queryParam(LIMIT_QUERY_PARAMETER, pageSize));
        return getDraftOrders(response);
    }

    public ShopifyPage<ShopifyDraftOrder> getDraftOrders(final DateTime mininumCreationDate) {
        return getDraftOrders(mininumCreationDate, DEFAULT_REQUEST_LIMIT);
    }

    public ShopifyPage<ShopifyDraftOrder> getDraftOrders(final DateTime mininumCreationDate, final int pageSize) {
        final Response response = get(buildDraftOrdersEndpoint().queryParam(STATUS_QUERY_PARAMETER, ANY_STATUSES)
                .queryParam(LIMIT_QUERY_PARAMETER, pageSize)
                .queryParam(CREATED_AT_MIN_QUERY_PARAMETER, mininumCreationDate.toString()));
        return getDraftOrders(response);
    ...
    public ShopifyPage<ShopifyDraftOrder> getDraftOrders(final DateTime mininumCreationDate, final DateTime maximumCreationDate) {
        return getDraftOrders(mininumCreationDate, maximumCreationDate, DEFAULT_REQUEST_LIMIT);
    }

    public ShopifyPage<ShopifyDraftOrder> getUpdatedDraftOrdersCreatedBefore(final DateTime minimumUpdatedAtDate,
            final DateTime maximumUpdatedAtDate, final DateTime maximumCreatedAtDate, final int pageSize) {
        final Response response = get(buildDraftOrdersEndpoint().queryParam(STATUS_QUERY_PARAMETER, ANY_STATUSES)
                .queryParam(LIMIT_QUERY_PARAMETER, pageSize)
                .queryParam(UPDATED_AT_MIN_QUERY_PARAMETER, minimumUpdatedAtDate.toString())
    ...
    public ShopifyPage<ShopifyDraftOrder> getDraftOrders(final DateTime mininumCreationDate, final DateTime maximumCreationDate,
            final String appId) {
        return getDraftOrders(mininumCreationDate, maximumCreationDate, appId, DEFAULT_REQUEST_LIMIT);
    }
    ...
    public ShopifyFulfillment updateFulfillment(final ShopifyFulfillmentUpdateRequest shopifyFulfillmentUpdateRequest) {
        ...
        final Response response = put(
                buildOrdersEndpoint().path(shopifyFulfillment.getOrderId()).path(FULFILLMENTS).path(shopifyFulfillment.getId()),
    ...
    public ShopifyFulfillment updateFulfillmentTrackingInfo(
        ...
        final Response response = post(getWebTarget().path(FULFILLMENTS).path(shopifyFulfillment.getId()).path(UPDATE_TRACKING),
                payload);
        final ShopifyFulfillmentRoot shopifyFulfillmentRootResponse = response.readEntity(ShopifyFulfillmentRoot.class);
        return shopifyFulfillmentRootResponse.getFulfillment();
    }
    ...
    public ShopifyDraftOrder createDraftOrder(final ShopifyDraftOrderCreationRequest shopifyDraftOrderCreationRequest) {
        final ShopifyDraftOrderRoot shopifyDraftOrderRoot = new ShopifyDraftOrderRoot();
        final ShopifyDraftOrder shopifyDraftOrder = shopifyDraftOrderCreationRequest.getRequest();
    ...
    public ShopifyFulfillment cancelFulfillment(final String orderId, final String fulfillmentId) {
        ...
        final Response response = post(buildOrdersEndpoint.path(orderId).path(FULFILLMENTS).path(fulfillmentId).path(CANCEL),
    ...
    public Metafield createVariantMetafield(final ShopifyVariantMetafieldCreationRequest shopifyVariantMetafieldCreationRequest) {
        ...
        final Response response = post(
                getWebTarget().path(VARIANTS).path(shopifyVariantMetafieldCreationRequest.getVariantId()).path(METAFIELDS),
                metafieldRoot);
    ...
    public Metafield createProductMetafield(final ShopifyProductMetafieldCreationRequest shopifyProductMetafieldCreationRequest) {
        ...
        final Response response = post(
                getWebTarget().path(PRODUCTS).path(shopifyProductMetafieldCreationRequest.getProductId()).path(METAFIELDS),
                metafieldRoot);
        final MetafieldRoot metafieldRootResponse = response.readEntity(MetafieldRoot.class);
        return metafieldRootResponse.getMetafield();
    }
    ...
    public String generateToken() {
        try {
            ...
            final Response response = this.getUnversionedWebTarget().path(OAUTH).path(ACCESS_TOKEN)
                    ...
                    .queryParam(AUTHORIZATION_CODE, this.authorizationToken).request(MediaType.APPLICATION_JSON).post(entity);
            ...
            if (attempt.hasResult()) {
                ...
                if (LOGGER.isWarnEnabled() && !hasExceededRateLimit(response) && shouldRetryResponse(response)) {
                    ...
                    LOGGER.warn(RETRY_INVALID_RESPONSE_ATTEMPT_MESSAGE, delaySinceFirstAttemptInSeconds,
                            attempt.getAttemptNumber(), response.getLocation(), response.getStatus(), response.getStringHeaders(),
                            responseBody);
                ...
                LOGGER.warn(RETRY_EXCEPTION_ATTEMPT_MESSAGE, attempt.getAttemptNumber(), delaySinceFirstAttemptInSeconds,
    ...
    public List<ShopifyFulfillmentOrder> getFulfillmentOrdersFromOrder(final String shopifyOrderId) {
        ...
        final ShopifyFulfillmentOrdersRoot shopifyFulfillmentOrdersRoot = response.readEntity(ShopifyFulfillmentOrdersRoot.class);
    ...
    public ShopifyFulfillmentOrder moveFulfillmentOrder(final String newLocationId,
        ...
        final Response response = post(getWebTarget().path(FULFILLMENT_ORDERS).path(shopifyFulfillmentOrder.getId()).path(MOVE),
                payload);
